...
export default function TokenWrapper() {
  ...
  useEffect(() => {
    const getWrapCoins = async () => {
      ...
      let query_wrap_token_list = await obelisk.query.wrapper_system.wrapped_assets(tx, params) as DevInspectResults
  ...
  const targetTokens = useMemo(() => {
    return assetMetadata.map(asset => {
      ...
      return {
        ...
        logo: <img src={asset.metadata[4]} alt={asset.metadata[1]} width="20" height="20" style={{marginRight: '8px'}} />,
      };
    });
  }, [assetMetadata]);
  ...
  const handleUnWrap = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    console.log("amountToUnwrap:", amountToUnwrap); // 调试日志

    if (isNaN(amountToUnwrap) || amountToUnwrap <= 0) {
      toast.error("无效金额", {
        description: "请输入一个有效的正数。",
      });
      return;
    }
    ...
    try {
      ...
      // 确保我们有有效的小数位数
      const decimals = selectedSourceToken.decimals || 9; // 如果没有小数位数信息,默认为9
      console.log("decimals:", decimals); // 调试日志

      // 使用 BigInt 来避免精度问题
      const amountInSmallestUnit = BigInt(Math.floor(amountToUnwrap * (10 ** decimals)));
      console.log("amountInSmallestUnit:", amountInSmallestUnit.toString()); // 调试日志
      
      if (amountInSmallestUnit <= 0) {
        throw new Error("无效的金额计算");
      }

      const unwrap_amount = tx.pure.u64(amountInSmallestUnit);
      ...
      let typeArguments = [`${targetToken}`];
      await obelisk.tx.wrapper_system.unwrap(tx, params, typeArguments, true);
                  ...
                  {currentSourceTokens.map((token) => (
                    <SelectItem key={token.value} value={token.value}>
                      <div className="flex items-center">
                        {token.logo}
                        <span>
                          {isWrap ? token.symbol : `P${token.symbol}`} ({token.balance} {isWrap ? token.symbol : `P${token.symbol}`})
                        </span>
                      </div>
                    </SelectItem>
                  ))}
                  ...
                  {currentTargetTokens.map((token) => (
                    <SelectItem key={token.value} value={token.value}>
                      <div className="flex items-center">
                        {token.logo}
                        <span>
                          {token.symbol} ({parseFloat(token.balance).toLocaleString(undefined, {minimumFractionDigits: 4, maximumFractionDigits: 4})} {token.symbol})
                        </span>
                      </div>
                    </SelectItem>
                  ))}
