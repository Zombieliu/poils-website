...
export default function Page({ params }: { params: { fromToken: string, toToken: string } }) {
    ...
    useEffect(() => {
        ...
        const fromTokenInfo = filteredAssets.find(asset => asset.metadata[1].toUpperCase() === fromTokenSymbol);
        const toTokenInfo = filteredAssets.find(asset => asset.metadata[1].toUpperCase() === toTokenSymbol);
    ...
    const getAmountOut = async (amount: string) => {
        ...
        let params: TransactionArgument[] = [
            tx.object(DEX_ID),
            tx.object(ASSETS_ID),
            tx.pure.vector('u32', [1, 0]),
            tx.pure.u64(parseFloat(amount) * 1e9), // Assuming 9 decimal places, adjust if needed
        ];
        let query_get_amount_out = await obelisk.query.dex_system.get_amount_out(tx, params) as DevInspectResults;
        let amount_out = obelisk.view(query_get_amount_out);
        return amount_out;
    };

    const calculateReceiveAmount = useCallback(debounce(async (amount: string) => {
